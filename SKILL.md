---
name: pcxa
description: PCXA construction intelligence platform CLI. Search/read files, manage tags and folders, manage activities/steps/progress/dependencies, manage form templates/fields/submissions, manage resources/timesheets/cost-codes/budgets, manage entity links between objects, and chat with the project's AI assistant for evaluation/probing. Use when the user asks about project files, documents, tasks, activities, forms, resources, timesheets, entity links, project management, or wants to send messages to the AI chatbot.
...
**Upload storage:** Files are uploaded to the company's configured storage provider (SharePoint or R2). If the company uses SharePoint, the file goes directly to SharePoint and a StorageReference is created — no separate sync needed. The 10MB Django upload limit applies; for larger files, use the presign flow (3-step: presign → upload to provider → create record).

**Deletion convention:** This CLI does **not** hard-delete files. `pcxa files delete <ids>` adds the `to_delete` tag — discoverable via `pcxa files list --tags to_delete` — and an out-of-band cleanup process performs the actual removal. Use this whenever you need to remove a file (e.g., a duplicate upload, a wrong version, an orphan upload artifact). Use `pcxa files restore <ids>` to undo before cleanup runs. Without `--yes`, `delete` prompts for confirmation.
...
Lock date ranges to prevent edits. Only staff can delete.
...
**Default JSON output** (from `chat send`) is structured for evaluators:
```json
{
  "conversation_id": 42,
  "agent_task_id": 9001,
...
**How it works:** the HTTP `send` endpoint persists the user message and dispatches an `AgentTask`; the actual reply streams asynchronously via WebSocket. This CLI polls `agent-tasks/{id}/` until the task hits a terminal status (`completed`, `failed`, `cancelled`, `waiting_for_input`), then fetches the conversation detail and returns the assistant message that came after the user message. Exit code 2 if the task ends in `failed`. `--no-wait` skips polling.

**Caveat:** for rigorous regression-style evaluation, the Django app already has a richer eval harness (`api/ai_services/management/commands/eval_chat_question.py`, `eval_smoke_gate`, `eval_telemetry_report`) that runs against synthetic corpora and persists `AgentEvalRun` rows with critic telemetry. `pcxa chat` is the right tool for ad-hoc probing or scripting an external evaluator agent against a real project; the management commands are the right tool for offline regression suites.

## Reporting API errors

When a PCXa CLI command returns a non-2xx API response (403, 500, unexpected 400 like "field required" when the field was provided, schema mismatches, etc.), treat it as a bug in the PCXa platform — not as an expected failure to route around.

**Required response:**
1. Surface the error to the user immediately with endpoint, status code, and response body.
2. Offer to file an issue at `https://github.com/PCX-Analytics/pcxa`.
3. On approval, file via `gh issue create --repo PCX-Analytics/pcxa` with:
   - **Title:** short, specific (status code + endpoint + condition).
   - **Body:** Summary, Reproduction (exact command + response), Impact (what workflow is blocked), Environment (CLI source, user, API URL, auth type), Expected behavior, Suggested check (if obvious), Workaround (if any).
4. Include the issue URL in the session log / activity comment so the bug is traceable from PCXa back to the tracker.
5. Only *after* the issue is filed should a workaround be used to continue the user's task. Workarounds must be marked as such in any logged comment (so future sessions don't mistake them for the intended workflow).

**Do not** silently retry, swallow errors, or invent alternate data paths without filing the issue first. PCXa is the source of truth for how the company runs — broken API behavior needs to be tracked, not worked around in the shadows.

**Known issues:** #133 (403 on comments and generic-links/create-attachment for company owners) — filed 2026-04-14.
